...
if __name__ == "__main__":
    ...
    CONFIG = SoccerPitchConfiguration()
    image = draw_pitch(CONFIG)

    # 仮に点を1個描いてみる（例: 中央サークル中心）
    ...
    # 中心点（X=0, Y=0）でテスト
    point = np.array([[0, 0]])
    image = draw_points_on_pitch(
        config=CONFIG, xy=point,
        face_color=Color.from_hex("#FF0000"),
        radius=10, pitch=image
    )

    # 表示
    cv2.imshow("Soccer Pitch", image)
    cv2.waitKey(0)
